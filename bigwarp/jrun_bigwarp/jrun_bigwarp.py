...
def bigwarp():
    bigwarp_base('bigwarp.BigWarp')

def bigwarp_base(main_class):
    argv               = sys.argv[1:]
    ...
    endpoint           = ['sc.fiji:bigwarp_fiji:3.1.2-SNAPSHOT:{}'.format(main_class)]
    bigwarp_argv       = argv if double_dash_index < 0 else argv[double_dash_index+1:]
    argv               = add_jvm_args_as_necessary(jrun_and_jvm_argv) + repository_strings + endpoint + bigwarp_argv
